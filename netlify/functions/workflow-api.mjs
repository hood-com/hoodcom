...
export const handler = async (event) => {
  ...
  try {
    ...
    if(action==='admin-user-dossier'){const admin=await adminToken(event);if(!admin)return json(403,{error:'صلاحية المدير مطلوبة'});const userId=String(body.userId||''),profile=await normalized.get('users',userId);if(!profile)return json(404,{error:'ملف المستخدم غير موجود'});const key=process.env.SUPABASE_SERVICE_ROLE_KEY,url=process.env.SUPABASE_URL,authResponse=await fetch(`${url}/auth/v1/admin/users/${encodeURIComponent(userId)}`,{headers:{apikey:key,authorization:`Bearer ${key}`}}),authUser=authResponse.ok?await authResponse.json():null;const collections=['user_balances','orders','deposit_requests','topup_transactions','notifications','activity','phone_verifications','order_private'];const entries=Object.fromEntries(await Promise.all(collections.map(async collection=>[collection,(await normalized.list(collection)).filter(item=>String(item.userId||item.id||'')===userId)])));return json(200,{ok:true,dossier:{profile,auth:{id:authUser?.id||userId,email:authUser?.email||profile.email||'',createdAt:authUser?.created_at||'',lastSignInAt:authUser?.last_sign_in_at||'',emailConfirmedAt:authUser?.email_confirmed_at||'',phoneConfirmedAt:authUser?.phone_confirmed_at||'',providers:(authUser?.identities||[]).map(i=>i.provider),role:authUser?.app_metadata?.role||'customer'},balance:entries.user_balances[0]||null,orders:entries.orders,deposits:entries.deposit_requests,topupTransactions:entries.topup_transactions,notifications:entries.notifications,activity:entries.activity,phoneVerification:entries.phone_verifications[0]||null,privateOrders:entries.order_private}});}
