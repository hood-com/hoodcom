...
const prepareCustomerFields=async(fields,orderId)=>{const result={};for(const[key,value]of Object.entries(fields||{})){if(value&&typeof value==='object'&&/^data:image\/(?:jpeg|png|webp);base64,/u.test(String(value.data||''))){const match=value.data.match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/su),ext={'image/jpeg':'jpg','image/png':'png','image/webp':'webp'}[match[1]],path=`orders/${orderId}/${String(key).replace(/[^a-zA-Z0-9_-]/gu,'-')}.${ext}`,service=process.env.SUPABASE_SERVICE_ROLE_KEY,url=process.env.SUPABASE_URL,response=await fetch(`${url}/storage/v1/object/catalog-images/${path}`,{method:'POST',headers:{apikey:service,authorization:`Bearer ${service}`,'content-type':match[1],'x-upsert':'false'},body:Buffer.from(match[2],'base64')});if(!response.ok)throw new Error('تعذر رفع ملف الطلب');result[key]={name:value.name,type:value.type,size:value.size,url:`${url}/storage/v1/object/public/catalog-images/${path}`};}else result[key]=cleanObject(value);}return result;};
