...
export const ProductCard = ({ product = {}, currency = 'YER', action = 'select-product' } = {}) => {
  ...
  return `<article class="item-card status-${escapeHTML(status)}" data-product-id="${escapeHTML(product.id || '')}">
    <div class="item-image-wrap"><img src="${escapeHTML(safeURL(/(?:^|\/)logo\.svg(?:[?#]|$)/iu.test(String(product.image || '')) ? '' : product.image, 'content-placeholder.svg'))}" alt="${escapeHTML(product.name || '')}" loading="lazy" decoding="async" fetchpriority="low" width="420" height="300"><span class="status-badge ${escapeHTML(status)}">${statusLabels[status] || status}</span></div>
