...
export const CategoryCard = ({ category = {}, href } = {}) => {
  ...
  return `<article class="category-card" data-category-id="${escapeHTML(category.id || '')}">
    <a href="${escapeHTML(target)}" class="category-card-link">
      <div class="category-image-wrap"><img src="${escapeHTML(image)}" alt="${escapeHTML(category.name || '')}" loading="lazy" decoding="async" fetchpriority="low" width="420" height="300"></div>
      <div class="category-info"><h3>${escapeHTML(category.name || 'قسم')}</h3><p>${escapeHTML(category.description || category.desc || '')}</p><span class="category-arrow">${icon('arrow', 18)}</span></div>
    </a>
  </article>`;
};
