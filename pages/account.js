...
const renderAccount = (user, orders) => {
  ...
  target.innerHTML = `<section class="account-hero"><div class="account-avatar">${escapeHTML((user.displayName || user.name || 'م').charAt(0))}</div><h1>${escapeHTML(user.displayName || user.name || 'المستخدم')}</h1><p>${escapeHTML(user.email || '')}</p><span class="account-status status-${escapeHTML(user.accountStatus || '')}">${escapeHTML(statusLabel(user.accountStatus))}</span></section>
    ...
    <section class="account-orders"><h2>طلباتي</h2><div id="userAccountOrdersContainer">${orders.length ? orders.map((order) => `<article class="order-card"><header><strong>${escapeHTML(order.itemName || order.offerName || order.id)}</strong><span class="status-${escapeHTML(order.status)}">${escapeHTML(statusLabel(order.status))}</span></header><p>${escapeHTML(order.offerName || '')}</p><footer><b>${formatPrice(order.total || order.price || 0, order.currency || 'YER')}</b><time>${escapeHTML(formatDate(order.createdAt))}</time></footer></article>`).join('') : '<div class="empty-state">لا توجد طلبات حتى الآن</div>'}</div></section>
