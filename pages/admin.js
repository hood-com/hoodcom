...
export const initAdminPage = async () => {
  ...
  try {
    await loadAll(); populateSettings(); populateTopupSettings(); renderAll(); installDepositAdmin(); installPurchaseSettingsEditor(); installPhoneVerificationCenter(); installPendingCenter(); bindTabs(); bindEditModal(); bindStaticImagePreviews(); bindAddForms(); bindSettingsForms(); bindTopupForms(); bindActions(); bindFeaturedDragDrop(); bindBackup();
    ...
    await startRealtime();
    globalThis.addEventListener('pagehide', () => { stopRealtime(); if(workflowTimer) clearInterval(workflowTimer); if(phoneVerificationTimer) clearInterval(phoneVerificationTimer); if(depositAdminTimer) clearInterval(depositAdminTimer); }, { once: true });
  } catch (error) {
