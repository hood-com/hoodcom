...
const bindActions = () => document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'delete-user' && globalThis.confirm('سيُحذف الحساب من Supabase Auth نهائياً. هل تريد المتابعة؟')) await saveAction(button, async () => { await deleteUserAccount(button.dataset.customerId); customers = customers.filter((entry) => entry.id !== button.dataset.customerId); orders = orders.filter((order) => String(order.userId) !== String(button.dataset.customerId)); renderCustomers(); }, 'تم حذف الحساب نهائياً ✅');
