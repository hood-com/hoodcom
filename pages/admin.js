...
import { getDepositConfig, listDepositsForAdmin, saveDepositMethod, updateDepositRequest } from '../services/deposit-service.js';
...
const renderDepositAdmin=async()=>{const methodsHost=byId('adminDepositMethods'),requestsHost=byId('adminDepositRequests');if(!methodsHost||!requestsHost)return;try{const[config,requests]=await Promise.all([getDepositConfig(),listDepositsForAdmin()]);methodsHost.innerHTML=config.methods.map(m=>`<article class="admin-list-item"><div class="admin-list-content"><strong>${escapeHTML(m.name)}</strong><small>${escapeHTML(m.accountNumber||'')} — ${m.enabled!==false?'مفعلة':'معطلة'}</small></div><button class="btn btn-sm" data-edit-deposit-method="${escapeAttr(m.id)}">تعديل</button></article>`).join('')||'<div class="empty-state">لا توجد طرق إيداع</div>';requestsHost.innerHTML=(requests.items||[]).map(r=>`<article class="workflow-row"><div class="workflow-row__main"><strong>${escapeHTML(r.methodName)}</strong><span>${escapeHTML(r.userName)} — ${escapeHTML(r.userPhone)}</span><small>${formatPrice(r.amount,r.currency)} — ${escapeHTML(formatDate(r.createdAt))}</small></div><div class="workflow-row__actions"><button class="btn btn-sm" data-deposit-action="approved" data-id="${escapeAttr(r.id)}">قبول</button><button class="btn btn-sm" data-deposit-action="processing" data-id="${escapeAttr(r.id)}">قيد التنفيذ</button><button class="btn btn-sm btn-danger" data-deposit-action="rejected" data-id="${escapeAttr(r.id)}">رفض</button></div><details><summary>كل التفاصيل</summary><pre>${escapeHTML(JSON.stringify(r,null,2))}</pre></details></article>`).join('')||'<div class="empty-state">لا توجد طلبات تغذية</div>';methodsHost.onclick=async e=>{const b=e.target.closest('[data-edit-deposit-method]');if(!b)return;const m=config.methods.find(x=>x.id===b.dataset.editDepositMethod);void editDepositMethod(m);};}catch(error){requestsHost.innerHTML=`<div class="error-message">${escapeHTML(error.message)}</div>`;}};
const editDepositMethod=async(current={})=>{try{const name=prompt('اسم المحفظة أو الخدمة',current.name||'');if(name===null)return;const id=current.id||prompt('معرف مختصر بالإنجليزية',`method-${Date.now()}`);const accountNumber=prompt('رقم الحساب',current.accountNumber||'');const accountName=prompt('اسم صاحب الحساب',current.accountName||'');const instructions=prompt('التعليمات',current.instructions||'');const minAmount=Number(prompt('الحد الأدنى',current.minAmount||0));const maxAmount=Number(prompt('الحد الأقصى',current.maxAmount||1000000));const receiptRequired=confirm('هل صورة إشعار التحويل مطلوبة؟');await saveDepositMethod({...current,id,name,accountNumber,accountName,instructions,minAmount,maxAmount,receiptRequired,enabled:true,currencies:current.currencies||['YER'],fields:current.fields||[],order:current.order||1});showToast('تم حفظ طريقة الإيداع ✅','success');await renderDepositAdmin();}catch(error){showToast(error.message,'error');}};
