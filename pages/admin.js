...
import { decideRequest, getPurchaseChannels, getUserDossier, listPendingForAdmin, requestNotificationPermission, savePurchaseChannels, systemNotify } from '../services/workflow-service.js';
import { decidePhoneVerification, listPhoneVerifications } from '../services/phone-verification-service.js';
import { deleteDepositMethod, getDepositConfig, listDepositsForAdmin, saveDepositMethod, updateDepositRequest } from '../services/deposit-service.js';
import { listNotificationsForAdmin, saveNotificationTemplate, sendNotification } from '../services/notification-service.js';
import { uploadManagedImage } from '../services/image-storage-service.js';
...
const dossierRows=(items,empty='لا توجد بيانات')=>items?.length?items.slice(0,100).map(item=>`<details class="dossier-record"><summary>${escapeHTML(item.itemName||item.methodName||item.type||item.title||item.id||'سجل')} — ${escapeHTML(formatDate(item.createdAt))}</summary><pre>${escapeHTML(JSON.stringify(item,null,2))}</pre></details>`).join(''):`<div class="empty-state">${empty}</div>`;
const openUserDossier=async(userId)=>{try{showToast('جاري تحميل الملف الشامل…','info');const{dossier}=await getUserDossier(userId),p=dossier.profile||{},a=dossier.auth||{},balance=Number(dossier.balance?.balance||0);setText('editModalTitle','الملف الشامل للعميل');byId('editModalBody').innerHTML=`<div class="user-dossier"><section><h3>الهوية والحساب</h3><dl><dt>UID</dt><dd dir="ltr">${escapeHTML(userId)}</dd><dt>الاسم</dt><dd>${escapeHTML(p.name||p.fullName||'')}</dd><dt>اسم المستخدم</dt><dd>${escapeHTML(p.username||'')}</dd><dt>الهاتف</dt><dd dir="ltr">${escapeHTML(p.phone||p.localPhone||'')}</dd><dt>البريد</dt><dd dir="ltr">${escapeHTML(a.email||'')}</dd><dt>العنوان</dt><dd>${escapeHTML([p.country,p.city,p.district,p.address].filter(Boolean).join(' — '))}</dd><dt>الحالة</dt><dd>${escapeHTML(p.accountStatus||'')}</dd><dt>الدور</dt><dd>${escapeHTML(a.role||'customer')}</dd><dt>تأكيد البريد</dt><dd>${escapeHTML(a.emailConfirmedAt||'غير مؤكد')}</dd><dt>آخر دخول</dt><dd>${escapeHTML(formatDate(a.lastSignInAt))}</dd><dt>المزود</dt><dd>${escapeHTML((a.providers||[]).join(', ')||'email')}</dd></dl></section><section><h3>البيانات المالية</h3><div class="dossier-stats"><b>الرصيد <span>${formatPrice(balance)}</span></b><b>المشتريات <span>${dossier.orders.length}</span></b><b>الإيداعات <span>${dossier.deposits.length}</span></b><b>الإشعارات <span>${dossier.notifications.length}</span></b></div></section><section><h3>توثيق الهاتف</h3>${dossier.phoneVerification?`<pre>${escapeHTML(JSON.stringify(dossier.phoneVerification,null,2))}</pre>`:'<p>لا يوجد طلب توثيق</p>'}</section><section><h3>طلبات الشراء</h3>${dossierRows(dossier.orders)}</section><section><h3>طلبات الإيداع</h3>${dossierRows(dossier.deposits)}</section><section><h3>الإشعارات</h3>${dossierRows(dossier.notifications)}</section><section><h3>سجل النشاط</h3>${dossierRows(dossier.activity)}</section><section><h3>المراجع الإدارية للطلبات</h3>${dossierRows(dossier.privateOrders)}</section></div>`;pendingEditorSave=null;if(byId('editSaveBtn'))byId('editSaveBtn').style.display='none';const modal=byId('editModal');modal?.classList.add('open');modal?.setAttribute('aria-hidden','false');document.body.classList.add('admin-editor-open');}catch(error){showToast(error.message,'error',{sticky:true});}};
...
const openEditModal = (title, body, onSave) => {
  if(byId('editSaveBtn'))byId('editSaveBtn').style.display='';
...
const bindEditModal = () => {
  ...
  byId('editModalBody')?.addEventListener('click', (event) => {
    ...
    if (action === 'add-deposit-field') byId('editDepositFields')?.insertAdjacentHTML('beforeend', dynamicFieldRow({}, byId('editDepositFields').children.length));
...
const bindActions = () => document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'user-dossier') await openUserDossier(button.dataset.customerId);
...
const renderPhoneVerifications=async()=>{const target=byId('phoneVerificationRows');if(!target)return;try{const{items}=await listPhoneVerifications();const query=String(byId('phoneVerificationSearch')?.value||'').trim().toUpperCase();const pending=(items||[]).filter((item)=>item.status==='pending'&&(!query||String(item.code||'').toUpperCase().includes(query)));setText('phoneVerificationCount',pending.length);target.innerHTML=pending.map((item)=>`<article class="workflow-row"><div class="workflow-row__main"><strong>${escapeHTML(item.name||'مستخدم')}</strong><span dir="ltr">${escapeHTML(item.phone||'')}</span><code>${escapeHTML(item.code||'')}</code><small>${escapeHTML(formatDate(item.createdAt))}</small></div><div class="workflow-row__actions"><button class="btn btn-sm" data-phone-decision="approve" data-user-id="${escapeAttr(item.userId)}">توثيق</button><button class="btn btn-sm btn-danger" data-phone-decision="cancel" data-user-id="${escapeAttr(item.userId)}">إلغاء</button></div></article>`).join('')||'<div class="empty-state">لا توجد طلبات توثيق هاتف</div>';}catch(error){target.innerHTML=`<div class="error-message">${escapeHTML(error.message)}</div>`;}};
const installPhoneVerificationCenter=()=>{const tabs=document.querySelector('.admin-tabs');if(!tabs||byId('tab-phone-verification'))return;const button=document.createElement('button');button.type='button';button.className='admin-tab';button.dataset.tab='phone-verification';button.innerHTML='📱 توثيق الهاتف <b id="phoneVerificationCount">0</b>';tabs.prepend(button);const panel=document.createElement('div');panel.className='admin-section';panel.id='tab-phone-verification';panel.innerHTML='<div class="admin-form"><h2>طلبات توثيق الهاتف</h2><p>قارن رقم المرسل برقم الحساب قبل التوثيق.</p><input id="phoneVerificationSearch" class="form-input" placeholder="ابحث بالرمز PHN-..." autocomplete="off"><div id="phoneVerificationRows"></div></div>';tabs.parentNode.insertBefore(panel,tabs.nextSibling);byId('phoneVerificationSearch')?.addEventListener('input',renderPhoneVerifications);panel.addEventListener('click',async(event)=>{const control=event.target.closest('[data-phone-decision]');if(!control)return;try{control.disabled=true;await decidePhoneVerification(control.dataset.userId,control.dataset.phoneDecision);showToast(control.dataset.phoneDecision==='approve'?'تم توثيق الحساب ✅':'تم إلغاء رمز التوثيق','success');await renderPhoneVerifications();}catch(error){showToast(error.message,'error');}finally{control.disabled=false;}});void renderPhoneVerifications();phoneVerificationTimer=setInterval(renderPhoneVerifications,5000);};
const installPendingCenter=()=>{
  ...
  byId('managePurchaseChannels')?.addEventListener('click',async()=>{try{let channels=await getPurchaseChannels(),action=prompt('اكتب: إضافة أو تعديل أو حذف','إضافة');if(!action)return;if(action.trim()==='حذف'){const id=prompt('معرف القناة المراد حذفها',channels[0]?.id||'');if(id)channels=channels.filter((c)=>c.id!==id);}else{const editing=action.trim()==='تعديل',id=prompt('معرف القناة',editing?(channels[0]?.id||''):`channel-${Date.now()}`);if(!id)return;const current=channels.find((c)=>c.id===id)||{};const name=prompt('اسم القناة',current.name||'واتساب');const type=prompt('النوع: whatsapp أو sms أو email أو telegram أو url',current.type||'whatsapp');const value=prompt('الرقم أو البريد أو الرابط',current.value||'');const forPurchase=confirm('هل تستخدم لإكمال الشراء؟');const forVerification=confirm('هل تستخدم لتوثيق الهاتف؟');const channel={...current,id,name,type,value,enabled:true,forPurchase,forVerification,order:current.order||channels.length+1};channels=editing?channels.map((c)=>c.id===id?channel:c):[...channels,channel];}await savePurchaseChannels(channels);showToast('تم حفظ القنوات ✅','success');}catch(error){showToast(error.message,'error',{sticky:true});}});
...
const installPurchaseSettingsEditor=()=>{const tab=byId('tab-settings');if(!tab||byId('purchasePageSettingsForm'))return;const current=getSiteSettings();const wrap=document.createElement('div');wrap.className='admin-form';wrap.innerHTML=`<h3>تخصيص صفحة تأكيد الشراء</h3><form id="purchasePageSettingsForm"><div class="form-group"><label>عنوان الصفحة</label><input class="form-input" name="title" value="${escapeAttr(current.purchasePageTitle||'تأكيد الطلب')}"></div><div class="form-group"><label>الوصف</label><textarea class="form-input" name="description">${escapeHTML(current.purchasePageDescription||'راجع تفاصيل طلبك ثم أكد العملية.')}</textarea></div><div class="form-group"><label>نص زر التأكيد</label><input class="form-input" name="confirmText" value="${escapeAttr(current.purchaseConfirmText||'تأكيد الطلب')}"></div><label><input type="checkbox" name="showBalance" ${current.purchaseShowBalance!==false?'checked':''}> إظهار الرصيد</label><label><input type="checkbox" name="showPrice" ${current.purchaseShowPrice!==false?'checked':''}> إظهار السعر</label><label><input type="checkbox" name="showDetails" ${current.purchaseShowDetails!==false?'checked':''}> إظهار تفاصيل المنتج</label><button class="btn btn-gold btn-full" type="submit">حفظ إعدادات الشراء</button></form>`;tab.prepend(wrap);wrap.querySelector('form').addEventListener('submit',async(event)=>{event.preventDefault();const form=event.currentTarget;try{await saveAction(event.submitter,()=>saveSiteSettings({purchasePageTitle:form.title.value,purchasePageDescription:form.description.value,purchaseConfirmText:form.confirmText.value,purchaseShowBalance:form.showBalance.checked,purchaseShowPrice:form.showPrice.checked,purchaseShowDetails:form.showDetails.checked}),'تم حفظ إعدادات صفحة الشراء ✅');}catch{}});};
let depositAdminTimer=null;
const renderDepositAdmin=async()=>{const methodsHost=byId('adminDepositMethods'),requestsHost=byId('adminDepositRequests');if(!methodsHost||!requestsHost)return;try{const[config,requests]=await Promise.all([getDepositConfig(),listDepositsForAdmin()]);methodsHost.innerHTML=config.methods.map(m=>`<article class="admin-list-item"><div class="admin-list-content"><strong>${escapeHTML(m.name)}</strong><small>${escapeHTML(m.accountNumber||'')} — ${m.enabled!==false?'مفعلة':'معطلة'}</small></div><div class="admin-list-actions"><button class="btn btn-sm" data-edit-deposit-method="${escapeAttr(m.id)}">تعديل</button><button class="btn btn-sm btn-danger" data-delete-deposit-method="${escapeAttr(m.id)}">حذف</button></div></article>`).join('')||'<div class="empty-state">لا توجد طرق إيداع</div>';requestsHost.innerHTML=(requests.items||[]).map(r=>`<article class="workflow-row"><div class="workflow-row__main"><strong>${escapeHTML(r.methodName)}</strong><span>${escapeHTML(r.userName)} — ${escapeHTML(r.userPhone)}</span><small>${formatPrice(r.amount,r.currency)} — ${escapeHTML(formatDate(r.createdAt))}</small></div><div class="workflow-row__actions"><button class="btn btn-sm" data-deposit-action="approved" data-id="${escapeAttr(r.id)}">قبول</button><button class="btn btn-sm" data-deposit-action="processing" data-id="${escapeAttr(r.id)}">قيد التنفيذ</button><button class="btn btn-sm btn-danger" data-deposit-action="rejected" data-id="${escapeAttr(r.id)}">رفض</button></div><details><summary>كل التفاصيل</summary><pre>${escapeHTML(JSON.stringify(r,null,2))}</pre></details></article>`).join('')||'<div class="empty-state">لا توجد طلبات تغذية</div>';methodsHost.onclick=async e=>{const edit=e.target.closest('[data-edit-deposit-method]'),del=e.target.closest('[data-delete-deposit-method]');if(edit){const m=config.methods.find(x=>x.id===edit.dataset.editDepositMethod);editDepositMethod(m);return;}if(del&&confirm('حذف طريقة الإيداع؟')){try{await deleteDepositMethod(del.dataset.deleteDepositMethod);showToast('تم حذف طريقة الإيداع','success');await renderDepositAdmin();}catch(error){showToast(error.message,'error');}}};}catch(error){requestsHost.innerHTML=`<div class="error-message">${escapeHTML(error.message)}</div>`;}};
const editDepositMethod=(current={})=>{openEditModal(current.id?'تعديل طريقة الإيداع':'إضافة طريقة إيداع',`<div class="admin-editor-grid"><label>المعرف<input class="form-input" name="id" value="${escapeAttr(current.id||`method-${Date.now()}`)}" ${current.id?'readonly':''} required></label><label>الاسم<input class="form-input" name="name" value="${escapeAttr(current.name||'')}" required></label><label>رقم الحساب<input class="form-input" name="accountNumber" value="${escapeAttr(current.accountNumber||'')}"></label><label>اسم صاحب الحساب<input class="form-input" name="accountName" value="${escapeAttr(current.accountName||'')}"></label><label>الحد الأدنى<input type="number" class="form-input" name="minAmount" value="${Number(current.minAmount||0)}"></label><label>الحد الأقصى<input type="number" class="form-input" name="maxAmount" value="${Number(current.maxAmount||1000000)}"></label><label>العملات بفواصل<input class="form-input" name="currencies" value="${escapeAttr((current.currencies||['YER']).join(','))}"></label><label>الترتيب<input type="number" class="form-input" name="order" value="${Number(current.order||1)}"></label><label class="admin-editor-span-2">التعليمات<textarea class="form-input" name="instructions">${escapeHTML(current.instructions||'')}</textarea></label><label class="admin-check"><input type="checkbox" name="enabled" ${current.enabled!==false?'checked':''}> مفعلة</label><label class="admin-check"><input type="checkbox" name="receiptRequired" ${current.receiptRequired!==false?'checked':''}> صورة الإشعار مطلوبة</label><label class="admin-check"><input type="checkbox" name="autoCredit" ${current.autoCredit===true?'checked':''}> إضافة الرصيد تلقائيًا</label></div>${imageEditor('editDepositImage',current.image,'صورة طريقة الإيداع')}<div class="admin-editor-section"><div class="admin-editor-section-head"><h4>حقول هذه الطريقة</h4><button type="button" class="btn btn-sm btn-outline" data-editor-action="add-deposit-field">+ إضافة حقل</button></div><div id="editDepositFields">${(current.fields||[]).map(dynamicFieldRow).join('')}</div></div>`,async(form)=>{let image=imageValue(form,'editDepositImagePreview',current.image||'');if(String(image).startsWith('data:image/'))image=await uploadManagedImage(image,'deposit-method',form.elements.id.value);await saveDepositMethod({...current,id:form.elements.id.value,name:form.elements.name.value,accountNumber:form.elements.accountNumber.value,accountName:form.elements.accountName.value,instructions:form.elements.instructions.value,minAmount:Number(form.elements.minAmount.value),maxAmount:Number(form.elements.maxAmount.value),currencies:form.elements.currencies.value.split(',').map(v=>v.trim()).filter(Boolean),order:Number(form.elements.order.value),enabled:form.elements.enabled.checked,receiptRequired:form.elements.receiptRequired.checked,autoCredit:form.elements.autoCredit.checked,image,fields:collectDynamicFields(form)});await renderDepositAdmin();});};
const installDepositAdmin=()=>{const tabs=document.querySelector('.admin-tabs');if(!tabs||byId('tab-deposit-management'))return;const button=document.createElement('button');button.type='button';button.className='admin-tab';button.dataset.tab='deposit-management';button.textContent='💳 تغذية حسابي';tabs.prepend(button);const panel=document.createElement('div');panel.className='admin-section';panel.id='tab-deposit-management';panel.innerHTML='<div class="admin-form"><div class="workflow-head"><h2>إدارة تغذية حسابي</h2><button class="btn btn-gold" id="addDepositMethod">إضافة طريقة إيداع</button></div><h3>طرق الإيداع</h3><div id="adminDepositMethods"></div><h3>طلبات الإيداع</h3><div id="adminDepositRequests"></div></div>';tabs.parentNode.insertBefore(panel,tabs.nextSibling);byId('addDepositMethod').onclick=()=>editDepositMethod({});panel.addEventListener('click',async e=>{const b=e.target.closest('[data-deposit-action]');if(!b)return;const status=b.dataset.depositAction,approvedAmount=status==='approved'?Number(prompt('المبلغ المعتمد','0')):undefined,reason=status==='rejected'?(prompt('سبب الرفض','')||''):'';try{b.disabled=true;await updateDepositRequest({id:b.dataset.id,status,approvedAmount,reason,creditBalance:status==='approved'&&confirm('إضافة المبلغ المعتمد إلى الرصيد؟')});showToast('تم تحديث الطلب ✅','success');await renderDepositAdmin();}catch(error){showToast(error.message,'error');}finally{b.disabled=false;}});void renderDepositAdmin();depositAdminTimer=setInterval(renderDepositAdmin,6000);};
let notificationAdminTimer=null;
const renderNotificationAdmin=async()=>{const list=byId('adminNotificationsList'),templates=byId('adminNotificationTemplates');if(!list||!templates)return;try{const data=await listNotificationsForAdmin();list.innerHTML=(data.items||[]).slice(0,100).map(item=>`<article class="admin-list-item"><div class="admin-list-content"><strong>${escapeHTML(item.title)}</strong><p>${escapeHTML(item.message)}</p><small>${item.audience==='all'?'جميع المستخدمين':escapeHTML(item.userId||'')} — ${escapeHTML(formatDate(item.createdAt))}</small></div></article>`).join('')||'<div class="empty-state">لا توجد إشعارات</div>';templates.innerHTML=(data.templates||[]).map(t=>`<article class="admin-list-item"><div class="admin-list-content"><strong>${escapeHTML(t.id)}</strong><p>${escapeHTML(t.title)} — ${escapeHTML(t.message)}</p></div><button class="btn btn-sm" data-edit-notification-template="${escapeAttr(t.id)}">تعديل</button></article>`).join('')||'<div class="empty-state">لا توجد قوالب</div>';templates.onclick=e=>{const b=e.target.closest('[data-edit-notification-template]');if(!b)return;const t=data.templates.find(x=>x.id===b.dataset.editNotificationTemplate);void editNotificationTemplate(t);};}catch(error){list.innerHTML=`<div class="error-message">${escapeHTML(error.message)}</div>`;}};
const editNotificationTemplate=async(template={})=>{const id=template.id||prompt('معرف القالب بالإنجليزية','custom_template');if(!id)return;const title=prompt('عنوان الإشعار',template.title||'');if(title===null)return;const message=prompt('نص الإشعار',template.message||'');if(message===null)return;const type=prompt('النوع: info, success, pending, error',template.type||'info');try{await saveNotificationTemplate({id,title,message,type,enabled:true});showToast('تم حفظ قالب الإشعار ✅','success');await renderNotificationAdmin();}catch(error){showToast(error.message,'error');}};
const installNotificationAdmin=()=>{const tabs=document.querySelector('.admin-tabs');if(!tabs||byId('tab-notifications-management'))return;const button=document.createElement('button');button.type='button';button.className='admin-tab';button.dataset.tab='notifications-management';button.textContent='🔔 الإشعارات';tabs.prepend(button);const panel=document.createElement('div');panel.className='admin-section';panel.id='tab-notifications-management';panel.innerHTML='<div class="admin-form"><h2>إرسال إشعار</h2><form id="adminSendNotificationForm" class="admin-editor-grid"><label>المستلم<select class="form-input" name="audience"><option value="all">جميع المستخدمين</option><option value="user">مستخدم محدد</option></select></label><label>معرف المستخدم<input class="form-input" name="userId" placeholder="UID عند اختيار مستخدم"></label><label>العنوان<input class="form-input" name="title" required></label><label>النوع<select class="form-input" name="type"><option value="info">معلومة</option><option value="success">نجاح</option><option value="pending">انتظار</option><option value="error">تنبيه</option></select></label><label class="admin-editor-span-2">النص<textarea class="form-input" name="message" required></textarea></label><label class="admin-editor-span-2">الرابط الاختياري<input class="form-input" name="link"></label><button class="btn btn-gold" type="submit">إرسال الإشعار</button></form><h3>قوالب الإشعارات</h3><button class="btn btn-outline" id="addNotificationTemplate">إضافة قالب</button><div id="adminNotificationTemplates"></div><h3>آخر الإشعارات</h3><div id="adminNotificationsList"></div></div>';tabs.parentNode.insertBefore(panel,tabs.nextSibling);const form=byId('adminSendNotificationForm');form.onsubmit=async e=>{e.preventDefault();try{await sendNotification({audience:form.audience.value,userId:form.userId.value,title:form.title.value,message:form.message.value,type:form.type.value,link:form.link.value});showToast('تم إرسال الإشعار ✅','success');form.reset();await renderNotificationAdmin();}catch(error){showToast(error.message,'error');}};byId('addNotificationTemplate').onclick=()=>editNotificationTemplate({});void renderNotificationAdmin();notificationAdminTimer=setInterval(renderNotificationAdmin,10000);};
...
export const initAdminPage = async () => {
  ...
  try {
    await loadAll(); populateSettings(); populateTopupSettings(); renderAll(); installNotificationAdmin(); installDepositAdmin(); installPurchaseSettingsEditor(); installPhoneVerificationCenter(); installPendingCenter(); bindTabs(); bindEditModal(); bindStaticImagePreviews(); bindAddForms(); bindSettingsForms(); bindTopupForms(); bindActions(); bindFeaturedDragDrop(); bindBackup();
    ...
    await startRealtime();
    globalThis.addEventListener('pagehide', () => { stopRealtime(); if(workflowTimer) clearInterval(workflowTimer); if(phoneVerificationTimer) clearInterval(phoneVerificationTimer); if(depositAdminTimer) clearInterval(depositAdminTimer); if(notificationAdminTimer) clearInterval(notificationAdminTimer); }, { once: true });
  } catch (error) {
